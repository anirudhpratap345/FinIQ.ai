...
function getPriorityColor(level: string): string {
  ...
  if (l === 'critical' || l === 'high') return 'bg-red-500/20 text-red-400 border-red-500/30';
  if (l === 'medium') return 'bg-amber-500/20 text-amber-400 border-amber-500/30';
  return 'bg-zinc-500/20 text-zinc-400 border-zinc-500/30';
}

function getPriorityBorderColor(level: string): string {
  const l = level?.toLowerCase() || '';
  if (l === 'critical' || l === 'high') return 'border-red-500';
  if (l === 'medium') return 'border-amber-500';
              ...
              <div className="grid grid-cols-3 md:grid-cols-6 gap-4 md:gap-6">
                {normalized.investors.map((investor, idx) => (
                  <div key={idx} className="flex flex-col items-center group">
                    <div className="w-16 h-16 md:w-20 md:h-20 rounded-xl bg-[#1a1a1a] border border-[#2a2a2a] flex items-center justify-center hover:-translate-y-1 transition-transform duration-200">
                      <span className="text-xl md:text-2xl font-bold text-zinc-500 group-hover:text-zinc-300 transition-colors">
