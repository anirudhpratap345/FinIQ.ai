...
export default function StrategyBlueprint({ data, onReset, formInputs, onDuplicate }: Props) {
  const [copySuccess, setCopySuccess] = useState(false);
  const [accuracyRating, setAccuracyRating] = useState(89);

  // Fetch accuracy rating for proof badge
  useEffect(() => {
    fetch('/api/stats')
      .then(res => res.json())
      .then(data => {
        if (data.accuracyRating) setAccuracyRating(data.accuracyRating);
      })
      .catch(() => {});
  }, []);
...
---
Generated by FinIQ in ${normalized.generationTime}s
`;
  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto bg-[#111111] border border-[#222222] rounded-2xl shadow-2xl overflow-hidden">
        ...
        {/* Gradient Header Bar */}
        <div className="h-16 md:h-20 bg-gradient-to-r from-[#1a1a2e] via-[#16213e] to-[#0f3460] flex items-center justify-between px-6 md:px-12">
          <div className="text-xl md:text-2xl font-bold text-white tracking-tight">FinIQ</div>
          <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-full px-3 py-1 text-xs text-emerald-400 font-medium hidden sm:block">
            {accuracyRating}% &quot;Scary Accurate&quot;
          </div>
        </div>
        ...
        <div className="py-12 md:py-16 px-8 md:px-12 space-y-12 md:space-y-16">
          ...
          <div>
            ...
            <p className="text-lg md:text-xl text-zinc-400 mt-4">
              Generated in {normalized.generationTime}s · Confidence: {normalized.confidence}
            </p>
          </div>

          {/* Funding Stage - Punchy Version */}
          <div className="text-zinc-300 space-y-3">
            <p className="text-2xl md:text-3xl font-medium text-white">
              You are <span className="text-emerald-400">{normalized.fundingStage}</span> ready
            </p>
            {oneLiner && (
              <p className="text-lg text-zinc-400">{oneLiner}</p>
            )}
          </div>
          ...
          {/* Raise Amount - Ultra Clean */}
          <div className="mt-16">
            <div className="text-6xl md:text-7xl lg:text-8xl font-light text-white tracking-tight">
              {normalized.raiseAmount.min} – {normalized.raiseAmount.max}
            </div>
            <div className="text-xl md:text-2xl text-zinc-400 mt-2">
              at {normalized.valuation.min}–{normalized.valuation.max} pre-money
            </div>
            <div className="text-lg text-zinc-500 italic mt-6 max-w-2xl">
              {normalized.raiseAmount.marketComps || "2025 reality: startups with these exact metrics are raising 2–3× this amount right now"}
            </div>
          </div>
          ...
          {/* Summary - Punchy One-Liner */}
          <div className="pt-8 border-t border-[#222222]">
            <p className="text-zinc-300 text-lg">
              Raise <span className="text-white font-medium">{normalized.raiseAmount.min}–{normalized.raiseAmount.max}</span>{" "}
              from {normalized.investors[0]?.name || 'target investors'} in the next 60–90 days →{" "}
              <span className="text-emerald-400">{normalized.runway.months} months runway</span>.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-[#0c0c0c]/95 backdrop-blur-sm border-t border-[#222222] py-4 md:py-6 z-50">
        <div className="max-w-4xl mx-auto px-4 flex items-center justify-center gap-3 md:gap-4 flex-wrap">
          <button
          ...
          {/* Duplicate & Tweak Scenario Button */}
          {formInputs && onDuplicate && (
            <button
              onClick={() => onDuplicate(formInputs)}
              className="px-4 md:px-6 py-2.5 md:py-3 rounded-lg border border-amber-600/50 text-amber-400 hover:bg-amber-900/20 hover:border-amber-500 transition-all text-sm md:text-base font-medium"
              title="Change one field and see how your strategy changes"
            >
              Duplicate & Tweak
            </button>
          )}
